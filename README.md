...
### Composants par Type de Nœud

| Type | Quantité | Rôle |
|------|----------|------|
| **Control Plane** | 3 | API Server, etcd, Controller Manager, Scheduler |
| **Infra Nodes** | 3 | Router/Ingress, Registry, Monitoring, Logging |
| **Worker Nodes** | 3 | Workloads applicatifs uniquement |
...
│   ├── configure-infra-nodes.sh        # Configuration labels/taints infra
│   └── modules/
│       ├── configure_dhcp.sh
│       ├── configure_dns.sh
...
# 6. (Post-installation) Configurer les nœuds infra
./scripts/configure-infra-nodes.sh
```

## 🏗️ Configuration des Nœuds Infra

Les nœuds Infrastructure sont dédiés aux composants système (Router, Registry, Monitoring).
Ils sont protégés par un **taint** qui empêche les workloads applicatifs de s'y exécuter.
...
```bash
# Configuration complète (labels, taints, migration des composants)
./scripts/configure-infra-nodes.sh

# Mode dry-run (simulation)
./scripts/configure-infra-nodes.sh --dry-run --verbose

# Seulement labels et taints (sans migration)
./scripts/configure-infra-nodes.sh --skip-migration
```

### Toleration pour les workloads

Pour qu'un pod puisse s'exécuter sur un nœud infra, il doit avoir cette toleration :
...
|-----------|-----------|-------------|
| Router/Ingress | openshift-ingress | Contrôleur d'entrée HTTP/HTTPS |
| Registry | openshift-image-registry | Registre d'images interne |
| Monitoring | openshift-monitoring | Prometheus, Alertmanager, Grafana |
...
nodes:
  ...
  # Nœuds Infrastructure (Router, Registry, Monitoring)
  infras:
...
| `infra` | Configuration des nœuds infra (labels/taints) | 5 min |
